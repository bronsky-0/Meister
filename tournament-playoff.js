...
(function(global) {
    ...
    function getServerNominations(remoteState) {
        if (!remoteState) {
            if (typeof NetworkSync !== 'undefined') {
                remoteState = NetworkSync.getState().lastRemoteState;
            }
        }
        if (!remoteState) return {};
        if (remoteState.nominations && Object.keys(remoteState.nominations).length) {
            return remoteState.nominations;
        }
        if (remoteState.tournament) {
            return { '': remoteState.tournament };
        }
        return {};
    }

    function getNominationSnapshotFromRemote(nominationId, remoteState) {
        var noms = getServerNominations(remoteState);
        if (nominationId && noms[nominationId]) return noms[nominationId];
        if (!nominationId && noms['']) return noms[''];
        if (!nominationId && remoteState && remoteState.tournament) return remoteState.tournament;
        return null;
    }

    function normalizeNominationId(nominationId) {
        return nominationId === '' || nominationId === undefined ? null : nominationId;
    }

    function nominationExistsOnServer(remoteState, nominationId) {
        return !!getNominationSnapshotFromRemote(normalizeNominationId(nominationId), remoteState);
    }

    function serverHasAnyTournamentData(remoteState) {
        if (!remoteState) return false;
        if (remoteState.tournament) return true;
        var noms = remoteState.nominations || {};
        return Object.keys(noms).length > 0;
    }

    function isAnyTournamentStartedOnServer(remoteState) {
        if (!remoteState) return false;
        if (remoteState.tournament && remoteState.tournament.playoffStarted) return true;
        var noms = remoteState.nominations || {};
        for (var id in noms) {
            if (!noms.hasOwnProperty(id)) continue;
            if (noms[id] && noms[id].playoffStarted) return true;
        }
        return false;
    }
    ...
    function formatDispatchError(err) {
        ...
        if (msg === 'Tournament not started' ||
            msg.indexOf('не синхронизирован с сервером') !== -1) {
            return 'Турнир ещё не синхронизирован с сервером. Дождитесь окончания синхронизации или нажмите «Начать турнир» ещё раз.';
        }
        return msg;
    }

    function resolveItemNominationId(item, options) {
        options = options || {};
        return normalizeNominationId(
            (item && item.nominationId) ||
            options.nominationId ||
            gameState.activeNominationId
        );
    }

    function listNetworkActiveNominationsForSync() {
        var result = [];
        var tournamentId = gameState.activeTournamentId;
        if (!tournamentId || typeof MeisterTournaments === 'undefined') {
            return result;
        }
        var tournament = MeisterTournaments.findTournament(tournamentId);
        if (!tournament || !tournament.nominations) return result;
        for (var i = 0; i < tournament.nominations.length; i++) {
            var nomination = tournament.nominations[i];
            if (nomination.networkActive && nomination.playoffStarted) {
    ...
    function ensureNominationSyncedBeforeClaim(nominationId) {
        ...
        if (nominationExistsOnServer(remote, nominationId)) {
            return Promise.resolve();
        }
        return syncAllNetworkActiveNominationsToServer();
    }

    function applySnapshotToGameState(snapshot, context) {
        context = context || {};
        if (!snapshot) return;
        if (context.nominationId) gameState.activeNominationId = context.nominationId;
        if (context.nominationName || snapshot.nominationName) {
            gameState.activeNominationName = context.nominationName || snapshot.nominationName || '';
        }
        gameState.sessionMode = snapshot.sessionMode || 'tournament';
        gameState.ruleset = snapshot.ruleset;
        gameState.tournamentSystem = snapshot.tournamentSystem;
        gameState.participants = snapshot.participants ? snapshot.participants.slice() : [];
        gameState.tournamentStage = snapshot.tournamentStage;
        gameState.pools = snapshot.pools ? JSON.parse(JSON.stringify(snapshot.pools)) : [];
        gameState.poolMatches = snapshot.poolMatches
            ? JSON.parse(JSON.stringify(snapshot.poolMatches))
            : {};
        gameState.bracket = snapshot.bracket ? JSON.parse(JSON.stringify(snapshot.bracket)) : null;
        gameState.playoffStarted = !!snapshot.playoffStarted;
        gameState.qualifyingAdvancersCount = snapshot.qualifyingAdvancersCount;
        gameState.tournamentFightHistory = snapshot.tournamentFightHistory
            ? snapshot.tournamentFightHistory.slice()
            : [];
        gameState.poolArenaAssignments = snapshot.poolArenaAssignments
            ? JSON.parse(JSON.stringify(snapshot.poolArenaAssignments))
            : {};
    ...
    function loadNominationContextForFight(nominationId, remoteState) {
        var snap = getNominationSnapshotFromRemote(nominationId, remoteState);
        if (!snap) return false;
        applySnapshotToGameState(snap, {
            nominationId: nominationId,
            nominationName: snap.nominationName
        });
        return true;
    }

    function getPoolAssignedArenaInSnapshot(snapshot, poolId) {
        if (!snapshot || !snapshot.poolArenaAssignments) return null;
        var val = snapshot.poolArenaAssignments[poolId];
        return val ? parseInt(val, 10) : null;
    }

    function nominationHasArenaAssignment(snapshot, arenaId) {
        arenaId = normalizeArenaId(arenaId);
        if (!snapshot || !arenaId) return false;
        var poolAssign = snapshot.poolArenaAssignments || {};
        for (var poolId in poolAssign) {
            if (!poolAssign.hasOwnProperty(poolId)) continue;
            if (normalizeArenaId(poolAssign[poolId]) === arenaId) return true;
        }
        var bracketAssign = snapshot.bracketArenaAssignments || {};
        for (var phaseKey in bracketAssign) {
            if (!bracketAssign.hasOwnProperty(phaseKey)) continue;
            if (normalizeArenaId(bracketAssign[phaseKey]) === arenaId) return true;
        }
        return false;
    }

    function getNominationsForArena(arenaId, remoteState) {
        var result = [];
        var noms = getServerNominations(remoteState);
        for (var nomId in noms) {
            if (!noms.hasOwnProperty(nomId)) continue;
            if (!nominationHasArenaAssignment(noms[nomId], arenaId)) continue;
            result.push({
                nominationId: nomId || null,
                snapshot: noms[nomId]
            });
        }
        return result;
    ...
    function isPoolStageSnapshot(snapshot) {
        return !!(snapshot && snapshot.playoffStarted && !snapshot.bracket);
    }

    function isBracketStageSnapshot(snapshot) {
        return !!(snapshot && snapshot.bracket &&
            (snapshot.tournamentStage === 'bracket' ||
                snapshot.tournamentStage === 'bracket-fights'));
    }
    ...
    /** Главное устройство: локально или подключено к серверу, но не терминал площадки. */
    function isTournamentHostDevice() {
        if (typeof NetworkSync === 'undefined') return true;
        if (!NetworkSync.isServerLinked()) return true;
        return !NetworkSync.isArena();
    }

    function shouldUseServerMatchSync() {
        return typeof NetworkSync !== 'undefined' &&
            NetworkSync.isServerLinked() &&
            !NetworkSync.isArena();
    }

    /** Хост в сети: только админ-панель, без секретарского табло (бои на площадках). */
    function isHostAdminOnlyMode() {
        return shouldUseServerMatchSync() && isTournamentHostDevice();
    }

    function hideSecretaryTerminal() {
        var term = document.getElementById('secretaryTerminal');
        if (term) term.classList.add('hidden');
    }

    function ensureHostAdminPanelVisible() {
        if (!isHostAdminOnlyMode() || !isPlayoffSessionActive()) return;
        hideSecretaryTerminal();
        document.getElementById('startMenuOverlay').style.display = 'none';
        document.getElementById('poolsOverlay').style.display = 'none';
        if (gameState.bracket) {
            showHostBracketManagementModal();
        } else if (gameState.playoffStarted) {
            showHostPoolManagementModal();
        }
    }

    function ensureHostRoleForOperations() {
        if (!shouldUseServerMatchSync()) return Promise.resolve();
    ...
    function mergePoolMatchesPreservingLocalDone(localPm, remotePm) {
        ...
        for (var poolId in localPm) {
            ...
            for (var j = 0; j < localMatches.length; j++) {
                ...
                } else if (lm.status === 'in_progress' &&
                    (rm.status === 'pending' || !rm.status)) {
                    var idx2 = merged[poolId].indexOf(rm);
                    merged[poolId][idx2] = JSON.parse(JSON.stringify(lm));
                }
            }
        }
        return merged;
    ...
    function refreshHostTournamentFromServer() {
        ...
        return NetworkSync.refreshServerState().then(function(state) {
            applyRemoteTournamentState(state, { skipAutoJoin: true, skipOverlayReopen: true });
            return state;
        });
    }

    /** Обновить открытый экран пулов/сетки без сброса детального вида и без перезагрузки с сервера. */
    function refreshPoolSelectOverlayUi() {
        var overlay = document.getElementById('poolSelectOverlay');
        if (!overlay || overlay.style.display !== 'flex') return;

        if (isNetworkArenaDevice()) return;

        if (gameState.bracket && isTournamentHostDevice()) {
            updatePoolSelectOverlayMode();
            var bTitle = document.getElementById('poolSelectTitle');
            var bArenaRow = document.getElementById('arenaSelectRow');
            if (bTitle) bTitle.textContent = 'Сетка турнира';
            if (bArenaRow) bArenaRow.style.display = 'none';
            if (currentDetailBracketPhaseKey) {
                showBracketPhaseDetailView(currentDetailBracketPhaseKey);
            } else {
                hideBracketDetailView();
        ...
        if (isTournamentHostDevice()) {
            ...
            var arenaSelect = document.getElementById('arenaSelectInput');
            populateArenaSelectElement(arenaSelect, readSelectedArenaId());
            if (currentDetailPoolId) {
                showPoolDetailView(currentDetailPoolId);
            } else {
                hidePoolDetailView();
    ...
    function parseMatchKey(matchKey) {
        ...
        if (matchKey.indexOf('nom:') === 0) {
            var poolMarker = ':pool:';
            var bracketMarker = ':bracket:';
            var poolIdx = matchKey.indexOf(poolMarker);
            if (poolIdx !== -1) {
                var nominationId = matchKey.slice(4, poolIdx);
                var poolRest = matchKey.slice(poolIdx + poolMarker.length);
                var poolSep = poolRest.indexOf(':');
                if (poolSep === -1) return null;
                return {
                    nominationId: nominationId,
                    type: 'pool',
                    poolId: poolRest.slice(0, poolSep),
                    matchId: poolRest.slice(poolSep + 1)
                };
            }
            var bracketIdx = matchKey.indexOf(bracketMarker);
            if (bracketIdx !== -1) {
                var nomId = matchKey.slice(4, bracketIdx);
                var bParts = matchKey.slice(bracketIdx + bracketMarker.length).split(':');
                ...
                return {
                    ...
                    bracketLoc: {
                        side: bParts[0],
                        roundIndex: parseInt(bParts[1], 10),
                        matchIndex: parseInt(bParts[2], 10)
                    }
                };
            }
            return null;
    ...
    function normalizeArenaId(arenaId) {
        var n = parseInt(arenaId, 10);
        return n > 0 ? n : null;
    }

    function buildMatchKeyFromParsed(parsed) {
        if (!parsed) return null;
        if (typeof NetworkSync !== 'undefined' && NetworkSync.buildMatchKey) {
            if (parsed.type === 'pool') {
                return NetworkSync.buildMatchKey(parsed.nominationId, 'pool', {
                    poolId: parsed.poolId,
                    matchId: parsed.matchId
                });
            }
            if (parsed.type === 'bracket' && parsed.bracketLoc) {
                return NetworkSync.buildMatchKey(parsed.nominationId, 'bracket', {
                    bracketLoc: parsed.bracketLoc
                });
            }
        }
        if (parsed.type === 'pool') {
            if (parsed.nominationId) {
                return 'nom:' + parsed.nominationId + ':pool:' + parsed.poolId + ':' + parsed.matchId;
            }
            return 'pool:' + parsed.poolId + ':' + parsed.matchId;
        }
        if (parsed.type === 'bracket' && parsed.bracketLoc) {
            var loc = parsed.bracketLoc;
            if (parsed.nominationId) {
                return 'nom:' + parsed.nominationId + ':bracket:' + loc.side + ':' +
    ...
    function findInProgressFightInSnapshot(snapshot, arenaId, nominationId) {
        arenaId = normalizeArenaId(arenaId);
        if (!snapshot || !arenaId) return null;

        var pools = snapshot.pools || [];
        var poolMatches = snapshot.poolMatches || {};
        for (var i = 0; i < pools.length; i++) {
            var poolId = pools[i].id;
            var matches = poolMatches[poolId] || [];
            for (var m = 0; m < matches.length; m++) {
                var pm = matches[m];
                if (pm.status === 'in_progress' && normalizeArenaId(pm.arenaId) === arenaId) {
                    return {
        ...
        var bracket = snapshot.bracket;
        if (!bracket) return null;

        function scanMatch(match, loc) {
            if (match && match.status === 'in_progress' &&
                normalizeArenaId(match.arenaId) === arenaId) {
                return {
                    nominationId: nominationId || null,
                    type: 'bracket',
                    bracketLoc: loc,
        ...
        var found = scanMatch(bracket.final, { side: 'final', roundIndex: 0, matchIndex: 0 });
        if (found) return found;
        found = scanMatch(bracket.thirdPlace, { side: 'bronze', roundIndex: 0, matchIndex: 0 });
        if (found) return found;

        for (var si = 0; si < 2; si++) {
            var side = si === 0 ? 'left' : 'right';
            var half = bracket[side];
            if (!half || !half.rounds) continue;
            for (var r = 0; r < half.rounds.length; r++) {
                for (var mi = 0; mi < half.rounds[r].length; mi++) {
                    found = scanMatch(half.rounds[r][mi], {
                        side: side,
                        roundIndex: r,
                        matchIndex: mi
                    });
    ...
    function findInProgressFightForArena(arenaId, remoteState) {
        arenaId = normalizeArenaId(arenaId);
        ...
        for (var nomId in noms) {
            if (!noms.hasOwnProperty(nomId)) continue;
            var foundRemote = findInProgressFightInSnapshot(noms[nomId], arenaId, nomId || null);
            if (foundRemote) return foundRemote;
        }

        if (gameState.playoffStarted) {
            return findInProgressFightInSnapshot(gameState, arenaId, gameState.activeNominationId);
        }
        return null;
    }
    ...
    function findHostAssignedFightForArena(remoteState, arenaId) {
        arenaId = normalizeArenaId(arenaId);
        if (!remoteState || !arenaId) return null;
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            if (!lock || normalizeArenaId(lock.arenaId) !== arenaId) continue;
            if (remoteState.hostDeviceId && lock.deviceId !== remoteState.hostDeviceId) continue;
            var parsed = parseMatchKey(matchKey);
            if (!parsed) continue;
            return { matchKey: matchKey, lock: lock, parsed: parsed };
        }

        var inProgress = findInProgressFightForArena(arenaId);
        if (!inProgress) return null;

        var fallbackKey = buildMatchKeyFromParsed(inProgress);
        if (!fallbackKey) return null;

        var fallbackLock = locks[fallbackKey] || null;
        if (remoteState.hostDeviceId && fallbackLock &&
            fallbackLock.deviceId !== remoteState.hostDeviceId) {
            return null;
        }
    ...
    function beginHostAssignedArenaFight(assignment) {
        ...
        var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
        if (parsed.nominationId) {
            loadNominationContextForFight(parsed.nominationId, remote);
        }
        if (parsed.type === 'pool') {
            var poolMatch = parsed.match || findPoolMatch(parsed.matchId);
            ...
            var loc = parsed.bracketLoc;
            var bracketMatch = parsed.match || getBracketMatch(loc);
    ...
    function getNextPendingPoolDispatchItemInSnapshot(snapshot, arenaId, nominationId) {
        arenaId = normalizeArenaId(arenaId);
        if (!snapshot || !snapshot.pools) return null;
        for (var i = 0; i < snapshot.pools.length; i++) {
            var poolId = snapshot.pools[i].id;
            if (getPoolAssignedArenaInSnapshot(snapshot, poolId) !== arenaId) continue;
            var matches = (snapshot.poolMatches && snapshot.poolMatches[poolId]) || [];
            for (var m = 0; m < matches.length; m++) {
                var match = matches[m];
    ...
    function getNextPendingPoolDispatchItem(arenaId) {
        arenaId = normalizeArenaId(arenaId);
        var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
        var noms = getServerNominations(remote);
        for (var nomId in noms) {
            if (!noms.hasOwnProperty(nomId)) continue;
            if (!isPoolStageSnapshot(noms[nomId])) continue;
            var item = getNextPendingPoolDispatchItemInSnapshot(
                noms[nomId],
                arenaId,
                normalizeNominationId(nomId)
            );
            if (item) return item;
        }
        if (gameState.playoffStarted && !gameState.bracket) {
            var activeId = gameState.activeNominationId;
            if (!activeId || !noms[activeId]) {
                if (!nominationExistsOnServer(remote, activeId)) {
                    return null;
                }
                return getNextPendingPoolDispatchItemInSnapshot(
                    gameState,
                    arenaId,
                    activeId
                );
    ...
    function collectAvailableBracketMatchesForHostInSnapshot(snapshot, arenaId, nominationId) {
        var items = [];
        if (!snapshot || !snapshot.bracket) return items;
        var savedBracket = gameState.bracket;
        var savedStage = gameState.tournamentStage;
        var savedPlayoff = gameState.playoffStarted;
        var savedBracketAssign = gameState.bracketArenaAssignments;
        gameState.bracket = snapshot.bracket;
        gameState.tournamentStage = snapshot.tournamentStage;
        gameState.playoffStarted = snapshot.playoffStarted;
        gameState.bracketArenaAssignments = snapshot.bracketArenaAssignments || {};
        ...
        function tryAdd(match, loc) {
            ...
            if (!isBracketPhaseOnArena(getBracketPhaseKeyFromLoc(loc), arenaId)) return;
        ...
        ['left', 'right'].forEach(function(side) {
            var half = snapshot.bracket[side];
        ...
        tryAdd(snapshot.bracket.thirdPlace, { side: 'bronze', roundIndex: 0, matchIndex: 0 });
        tryAdd(snapshot.bracket.final, { side: 'final', roundIndex: 0, matchIndex: 0 });

        gameState.bracket = savedBracket;
        gameState.tournamentStage = savedStage;
        gameState.playoffStarted = savedPlayoff;
        gameState.bracketArenaAssignments = savedBracketAssign;
        return items;
    }
    ...
    function collectAvailableBracketMatchesForHost(arenaId) {
        ...
        for (var nomId in noms) {
            if (!noms.hasOwnProperty(nomId)) continue;
            if (!isBracketStageSnapshot(noms[nomId])) continue;
            items = items.concat(collectAvailableBracketMatchesForHostInSnapshot(
                noms[nomId],
                arenaId,
                nomId || null
            ));
        ...
        var activeId = gameState.activeNominationId;
        if (gameState.bracket && isBracketStageActive() && (!activeId || !noms[activeId])) {
            items = items.concat(collectAvailableBracketMatchesForHostInSnapshot(
                gameState,
                arenaId,
                activeId
    ...
    function getNextDispatchItemForArena(arenaId) {
        var poolItem = getNextPendingPoolDispatchItem(arenaId);
        if (poolItem) return poolItem;
        return getNextBracketDispatchItem(arenaId);
    }

    function arenaHasPendingPoolWork(arenaId, remoteState) {
        arenaId = normalizeArenaId(arenaId);
        var assigned = getNominationsForArena(arenaId, remoteState);
        for (var i = 0; i < assigned.length; i++) {
            if (!isPoolStageSnapshot(assigned[i].snapshot)) continue;
            if (!areArenaPoolFightsCompleteForSnapshot(assigned[i].snapshot, arenaId)) {
                return true;
            }
        }
        return isPoolStageActive() && arenaHasAssignedPool(arenaId, remoteState) &&
            !areArenaPoolFightsComplete(arenaId);
    }

    function arenaHasPendingBracketWork(arenaId, remoteState) {
        arenaId = normalizeArenaId(arenaId);
        var assigned = getNominationsForArena(arenaId, remoteState);
        for (var i = 0; i < assigned.length; i++) {
            if (!isBracketStageSnapshot(assigned[i].snapshot)) continue;
            if (!areArenaBracketFightsCompleteForSnapshot(
                assigned[i].snapshot,
                arenaId,
                remoteState
            )) {
        ...
        return isBracketStageActive() && arenaHasAssignedBracketPhase(arenaId, remoteState) &&
            !areArenaBracketFightsComplete(arenaId);
    }

    function arenaHasAssignedPool(arenaId, remoteState) {
        arenaId = normalizeArenaId(arenaId);
        if (!arenaId) return false;
        var assigned = getNominationsForArena(arenaId, remoteState);
        for (var i = 0; i < assigned.length; i++) {
            if (isPoolStageSnapshot(assigned[i].snapshot)) return true;
        }
        if (isPoolStageActive()) {
            for (var p = 0; p < gameState.pools.length; p++) {
                if (getPoolAssignedArena(gameState.pools[p].id) === arenaId) return true;
            }
        }
        return false;
    }
    ...
    function areArenaPoolFightsCompleteForSnapshot(snapshot, arenaId) {
        arenaId = normalizeArenaId(arenaId);
        var hasAssigned = false;
        if (!snapshot || !snapshot.pools) return false;
        for (var i = 0; i < snapshot.pools.length; i++) {
            var poolId = snapshot.pools[i].id;
            if (getPoolAssignedArenaInSnapshot(snapshot, poolId) !== arenaId) continue;
            hasAssigned = true;
    ...
    function areArenaPoolFightsComplete(arenaId) {
        return areArenaPoolFightsCompleteForSnapshot(gameState, arenaId);
    }
    ...
    function arenaHadBracketFights(arenaId) {
        ...
        for (var p = 0; p < phases.length; p++) {
            if (getBracketAssignedArena(phases[p].key) !== arenaId) continue;
            var entries = getBracketMatchesInPhase(phases[p].key);
            for (var i = 0; i < entries.length; i++) {
                var match = entries[i].match;
                if (match && (match.status === 'done' || match.status === 'in_progress') &&
                    normalizeArenaId(match.arenaId) === arenaId) {
                    return true;
                }
            }
    ...
    function areArenaBracketFightsComplete(arenaId) {
        ...
        arenaId = normalizeArenaId(arenaId);
        if (!arenaHasAssignedBracketPhase(arenaId)) return false;
    ...
    function arenaHasAssignedBracketPhase(arenaId, remoteState) {
        arenaId = normalizeArenaId(arenaId);
        if (!arenaId) return false;
        var assigned = getNominationsForArena(arenaId, remoteState);
        for (var i = 0; i < assigned.length; i++) {
            if (isBracketStageSnapshot(assigned[i].snapshot)) return true;
        }
        if (isBracketStageActive() && gameState.bracketArenaAssignments) {
            var keys = Object.keys(gameState.bracketArenaAssignments);
            for (var k = 0; k < keys.length; k++) {
                if (normalizeArenaId(gameState.bracketArenaAssignments[keys[k]]) === arenaId) {
                    return true;
                }
            }
        }
    ...
    function isArenaStageCompleteForTerminal() {
        ...
        if (!arenaId) return false;
        var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
        var assigned = getNominationsForArena(arenaId, remote);
        if (!assigned.length) {
        ...
        for (var i = 0; i < assigned.length; i++) {
            var snap = assigned[i].snapshot;
            if (isPoolStageSnapshot(snap)) {
                if (!areArenaPoolFightsCompleteForSnapshot(snap, arenaId)) return false;
            } else if (isBracketStageSnapshot(snap)) {
                if (!areArenaBracketFightsCompleteForSnapshot(snap, arenaId, remote)) return false;
            }
        }
        return true;
    }

    function areArenaBracketFightsCompleteForSnapshot(snapshot, arenaId, remoteState) {
        if (!isBracketStageSnapshot(snapshot) || !arenaId) return false;
        arenaId = normalizeArenaId(arenaId);
        if (!nominationHasArenaAssignment(snapshot, arenaId)) return false;
        if (findInProgressFightInSnapshot(snapshot, arenaId, null)) return false;
        if (remoteState && findHostAssignedFightForArena(remoteState, arenaId)) return false;
        var items = collectAvailableBracketMatchesForHostInSnapshot(snapshot, arenaId, null);
    ...
    function shouldArenaShowWaitScreen(arenaId, remoteState) {
        ...
        if (findInProgressFightForArena(arenaId, remoteState)) return false;

        var assigned = getNominationsForArena(arenaId, remoteState);
        if (!assigned.length) return true;
        if (isArenaStageCompleteForTerminal()) return true;
        return false;
    }
    ...
    function handleArenaPostFightUi(remoteState) {
        ...
        return tryAutoJoinHostAssignedFight(remoteState).then(function(joined) {
            ...
            if (arenaHasPendingPoolWork(arenaId, remoteState)) {
                hideArenaWaitOverlay();
                hideArenaStageCompleteOverlay();
                if (term) term.classList.add('hidden');
            ...
            if (arenaHasPendingBracketWork(arenaId, remoteState)) {
                hideArenaWaitOverlay();
                hideArenaStageCompleteOverlay();
                if (term) term.classList.add('hidden');
    ...
    function hostDispatchClaim(item, arenaId, options) {
        ...
        options = options || {};
        var nominationId = resolveItemNominationId(item, options);
        if (nominationId) options.nominationId = nominationId;
        return ensureNominationSyncedBeforeClaim(nominationId).then(function() {
            var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
            if (!nominationExistsOnServer(remote, nominationId)) {
                return null;
            }
    ...
    function dispatchNextFightForArena(arenaId) {
        if (!isTournamentHostDevice() || !shouldUseServerMatchSync() || !arenaId || hostDispatchInFlight) {
            return Promise.resolve();
        }
        if (!isNetworkHost()) {
        ...
        if (findInProgressFightForArena(arenaId, remote)) {
            return Promise.resolve();
        }
        ...
        var dispatchNominationId = resolveItemNominationId(item);
        return hostDispatchClaim(item, arenaId).then(function(data) {
            if (data && data.state) {
                applyRemoteTournamentState(data.state);
            }
        }).catch(function(err) {
            var notSynced = err && err.message &&
                (err.message.indexOf('не синхронизирован') !== -1 ||
                    err.message === 'Tournament not started');
            if (notSynced) {
                return syncAllNetworkActiveNominationsToServer().then(function() {
                    return hostDispatchClaim(item, arenaId);
                }).then(function(data) {
                    if (data && data.state) {
                        applyRemoteTournamentState(data.state);
                    }
            ...
            if (err && err.message && err.message.indexOf('Match already done') === -1) {
                alert('Не удалось назначить бой на площадку ' + arenaId +
                    (dispatchNominationId ? ' (номинация ' + dispatchNominationId + ')' : '') +
                    ': ' + formatDispatchError(err));
            }
        }).finally(function() {
    ...
    function collectArenaIdsFromSnapshot(snapshot, ids) {
        if (!snapshot) return;
        var poolAssign = snapshot.poolArenaAssignments || {};
        for (var poolId in poolAssign) {
            if (!poolAssign.hasOwnProperty(poolId)) continue;
            var a = normalizeArenaId(poolAssign[poolId]);
            if (a) ids[a] = true;
        }
        var bracketAssign = snapshot.bracketArenaAssignments || {};
        for (var phaseKey in bracketAssign) {
            if (!bracketAssign.hasOwnProperty(phaseKey)) continue;
            var b = normalizeArenaId(bracketAssign[phaseKey]);
            if (b) ids[b] = true;
        }
    }
    ...
    function getArenaIdsForAutoDispatch() {
        ...
        for (var nomId in noms) {
            if (!noms.hasOwnProperty(nomId)) continue;
            collectArenaIdsFromSnapshot(noms[nomId], ids);
        }
        if (gameState.playoffStarted) {
            collectArenaIdsFromSnapshot(gameState, ids);
        }
        return Object.keys(ids).map(function(k) { return parseInt(k, 10); });
    }

    function dispatchForAllActiveArenas() {
        if (!isTournamentHostDevice() || !shouldUseServerMatchSync()) return Promise.resolve();
        if (!isNetworkHost()) {
            return ensureHostRoleForOperations().then(function() {
    ...
    function tryAutoJoinHostAssignedFight(remoteState) {
        ...
        if (assignment.parsed.nominationId) {
            loadNominationContextForFight(assignment.parsed.nominationId, remoteState);
        }
        ...
        return NetworkSync.joinMatch(assignment.matchKey, assignment.parsed.nominationId).then(function(data) {
            if (data && data.state) {
    ...
    function updateArenaWaitOverlay(remoteState, assignment) {
        ...
        if (!remoteState || !isAnyTournamentStartedOnServer(remoteState)) {
            return;
        }
        ...
        if (!assignment) {
            ...
            if (!isAnyTournamentStartedOnServer(remoteState)) {
        ...
        var nomLabel = '';
        if (assignment.parsed.nominationId) {
            var nomSnap = getNominationSnapshotFromRemote(
                assignment.parsed.nominationId,
                remoteState
            );
            if (nomSnap && nomSnap.nominationName) {
                nomLabel = nomSnap.nominationName + ': ';
            }
        }
        ...
        hintEl.textContent = 'Главное устройство запустило: ' + nomLabel + label;
    }
    ...
    function pushNominationSnapshotToServer(nomination, tournamentId) {
        var snapshot;
        if (nomination.id === gameState.activeNominationId) {
            snapshot = NetworkSync.getTournamentSnapshot(gameState);
        } else if (typeof MeisterTournaments !== 'undefined' &&
            MeisterTournaments.buildNetworkSnapshotFromNomination) {
            snapshot = MeisterTournaments.buildNetworkSnapshotFromNomination(nomination);
        }
        if (!snapshot) return Promise.resolve(null);
        return NetworkSync.pushTournament(snapshot, {
            nominationId: nomination.id,
            tournamentId: tournamentId
        });
    }

    function syncAllNetworkActiveNominationsToServer() {
        if (typeof global.persistActiveTournamentNominationIfAny === 'function') {
            global.persistActiveTournamentNominationIfAny();
        }
        ...
        var tournamentId = gameState.activeTournamentId;
        var nominations = listNetworkActiveNominationsForSync();
        if (!nominations.length) {
            return syncTournamentToServer().then(function() { return true; });
        }
            ...
            for (var i = 0; i < nominations.length; i++) {
                (function(nom) {
                    chain = chain.then(function() {
                        return pushNominationSnapshotToServer(nom, tournamentId);
                    });
                })(nominations[i]);
            }
            return chain;
    ...
    function syncTournamentToServer() {
        ...
        return NetworkSync.ensureHostRegistered(arenaCount).then(function() {
            return NetworkSync.pushTournament(NetworkSync.getTournamentSnapshot(gameState), {
                nominationId: gameState.activeNominationId,
                tournamentId: gameState.activeTournamentId
            });
        }).then(function(data) {
            if (data && data.state) {
                applyRemoteTournamentState(data.state, {
    ...
    function syncTournamentAfterFight() {
        ...
        tournamentSyncPending = true;
        return NetworkSync.completeMatch(NetworkSync.getTournamentSnapshot(gameState), {
            nominationId: gameState.activeNominationId
        }).then(function(data) {
            if (data && data.state) {
                applyRemoteTournamentState(data.state, {
    ...
    function populateNetworkArenaSelects() {
        if (!shouldUseServerMatchSync()) return;
    ...
    function onAdminArenaChanged() {
        ...
        if (isTournamentHostDevice() && shouldUseServerMatchSync()) {
    ...
    function resolveRemoteTournamentPayload(remoteState) {
        if (!remoteState) return null;
        var activeNomId = gameState.activeNominationId;
        if (remoteState.nominations && Object.keys(remoteState.nominations).length) {
            if (activeNomId && remoteState.nominations[activeNomId]) {
                return remoteState.nominations[activeNomId];
            }
            return null;
        }
        return remoteState.tournament || null;
    }

    function applyRemoteTournamentState(remoteState, options) {
        options = options || {};
        if (!remoteState) return;
        var hasNominations = remoteState.nominations &&
            Object.keys(remoteState.nominations).length > 0;
        if (!hasNominations && !remoteState.tournament) return;
        ...
        if (tournamentSyncPending && !options.fromHostPush) {
            return;
        }

        if (isNetworkArenaDevice()) {
            var inFightArena = !!(gameState.activePoolMatchId || gameState.activeBracketMatch);
            if (inFightArena && gameState.activeNominationId) {
                var fightSnap = getNominationSnapshotFromRemote(
                    gameState.activeNominationId,
                    remoteState
                );
                if (fightSnap) {
                    applySnapshotToGameState(fightSnap, {
                        nominationId: gameState.activeNominationId,
                        nominationName: fightSnap.nominationName
                    });
                }
            }
            updateTournamentBar();
            ...
            updatePlayoffTerminalButtons();
            if (!options.skipAutoJoin) {
                handleArenaPostFightUi(remoteState);
            }
            return;
        }

        var t = resolveRemoteTournamentPayload(remoteState);
        if (!t) {
            if (!options.skipAutoJoin && isNetworkHost() &&
                serverHasAnyTournamentData(remoteState)) {
        ...
        if (options.fromHostPush) {
            gameState.poolArenaAssignments = t.poolArenaAssignments || {};
            gameState.bracketArenaAssignments = t.bracketArenaAssignments || {};
        } else {
            gameState.poolArenaAssignments = Object.assign(
                {},
                gameState.poolArenaAssignments || {},
                t.poolArenaAssignments || {}
            );
            gameState.bracketArenaAssignments = Object.assign(
                {},
                gameState.bracketArenaAssignments || {},
                t.bracketArenaAssignments || {}
            );
        }
    ...
    function showSecretaryTerminal() {
        if (isHostAdminOnlyMode()) {
            hideSecretaryTerminal();
            ensureHostAdminPanelVisible();
            return;
        }
        document.getElementById('secretaryTerminal').classList.remove('hidden');
    }
    ...
    function isBracketStageActive() {
        ...
        var canForm = allPoolsComplete() && !gameState.bracket &&
            (!shouldUseServerMatchSync() || isTournamentHostDevice());
    ...
    function updatePlayoffTerminalButtons() {
        ...
        if (arenaWrap) {
            if (shouldUseServerMatchSync() && playoffOn && isTournamentHostDevice()) {
    ...
    function updateOpenBracketButton() {
        ...
        if (gameState.bracket) {
            btn.classList.remove('hidden');
            btn.textContent = isHostAdminOnlyMode() ? 'Показать сетку' : 'Схема сетки';
        } else {
            btn.classList.add('hidden');
        }
    }

    function getBracketDisplayUrl() {
        var serverUrl = '';
        if (typeof NetworkSync !== 'undefined' && NetworkSync.getState().serverUrl) {
            serverUrl = NetworkSync.getState().serverUrl.replace(/\/$/, '');
        } else {
            try {
                serverUrl = (localStorage.getItem('meisterServerUrl') || '').replace(/\/$/, '');
            } catch (e) {
                serverUrl = '';
            }
        }
        if (serverUrl) {
            return serverUrl + '/secretary_terminal.html?view=bracket';
        }
        if (window.location.protocol === 'http:' || window.location.protocol === 'https:') {
            var path = window.location.pathname.replace(/[^/]+$/, 'secretary_terminal.html');
            return window.location.origin + path + '?view=bracket';
        }
        return 'http://127.0.0.1:41235/secretary_terminal.html?view=bracket';
    }

    function openBracketDisplayWindow() {
        var url = getBracketDisplayUrl();
        var win = window.open(url, 'meister-bracket-display', 'noopener,noreferrer');
        if (!win) {
            alert('Разрешите всплывающие окна для просмотра сетки в отдельной вкладке.');
        }
    }

    function applyBracketDisplayFromRemote(remoteState) {
        if (!remoteState) return;
        var t = resolveRemoteTournamentPayload(remoteState);
        if (!t && remoteState.nominations) {
            var nomIds = Object.keys(remoteState.nominations);
            if (nomIds.length === 1) {
                t = remoteState.nominations[nomIds[0]];
            } else {
                for (var i = 0; i < nomIds.length; i++) {
                    var cand = remoteState.nominations[nomIds[i]];
                    if (cand && cand.bracket) {
                        t = cand;
                        break;
                    }
                }
        ...
        gameState.tournamentStage = t.tournamentStage;
        gameState.playoffStarted = !!t.playoffStarted;
        gameState.tournamentSystem = t.tournamentSystem || 'playoff';
        gameState.sessionMode = t.sessionMode || 'tournament';
        gameState.activeNominationName = t.nominationName || '';
        renderBracket(true);
        var titleEl = document.querySelector('.bracket-brand-title');
        if (titleEl) {
            var title = t.nominationName ? t.nominationName + ' — сетка' : 'Сетка Meister';
            titleEl.textContent = gameState.bracket ? title : 'Ожидание сетки';
        }
    }

    function initBracketDisplayView() {
        document.body.classList.add('bracket-display-only');
        document.title = 'Meister — сетка турнира';

        var idsToHide = [
            'startMenuOverlay', 'poolsOverlay', 'poolSelectOverlay', 'poolRankingsOverlay',
            'adminServerLogOverlay', 'secretaryTerminal', 'arenaWaitOverlay',
            'arenaStageCompleteOverlay', 'timeUpOverlay'
        ];
        ...
        var backBtn = document.querySelector('#bracketOverlay .menu-button.back');
        if (backBtn) backBtn.style.display = 'none';
        var arenaWrap = document.getElementById('bracketArenaWrap');
        if (arenaWrap) arenaWrap.style.display = 'none';
        var logBtn = document.getElementById('openServerLogBtnBracket');
        if (logBtn) logBtn.style.display = 'none';

        openBracketOverlay();

        if (typeof NetworkSync === 'undefined') return;

        function subscribeBracketDisplay() {
            NetworkSync.onStateUpdate(function(state) {
                applyBracketDisplayFromRemote(state);
            });
        }

        subscribeBracketDisplay();
        NetworkSync.autoConnectIfSaved().then(function() {
            return NetworkSync.refreshServerState();
        }).then(function(fresh) {
            if (fresh) applyBracketDisplayFromRemote(fresh);
        }).catch(function() {
            /* offline or server not ready */
        });
    }

    function maybeRunBracketDisplayViewFromUrl() {
        var params = new URLSearchParams(window.location.search);
        if (params.get('view') !== 'bracket') return false;
        initBracketDisplayView();
        return true;
    }

    function openBracketSchemeView() {
        if (!gameState.bracket) {
            alert('Сетка ещё не сформирована.');
            return;
        }
        if (isHostAdminOnlyMode()) {
            openBracketDisplayWindow();
            return;
        }
        hidePoolSelectModal();
        renderBracket();
        ...
        for (var i = 0; i < gameState.pools.length; i++) {
            ...
            for (var m = 0; m < matches.length; m++) {
                ...
                items.push({
                    ...
                    matchId: match.id,
                    match: match,
                    nominationId: gameState.activeNominationId || null,
    ...
    function collectAvailableBracketMatches(arenaId) {
        ...
        function tryAdd(match, loc) {
            if (!match) return;
            var phaseKey = getBracketPhaseKeyFromLoc(loc);
            if (!isBracketPhaseOnArena(phaseKey, arenaId)) return;
            if (isNetworkArenaDevice()) {
                if (match.status !== 'in_progress' || normalizeArenaId(match.arenaId) !== normalizeArenaId(arenaId)) {
                    return;
                }
                ...
                if (normalizeArenaId(match.arenaId) !== normalizeArenaId(arenaId)) return;
    ...
    var currentDetailBracketPhaseKey = null;
    var currentArenaViewNominationId = null;

    function ensureBracketArenaAssignments() {
        if (!gameState.bracketArenaAssignments) {
            gameState.bracketArenaAssignments = {};
        }
    }
    ...
    function enumerateBracketPhases() {
        var phases = [];
    ...
    function getBracketMatchesInPhase(phaseKey) {
        var result = [];
        ...
        } else if (phaseKey === 'bronze') {
            result.push({
                match: gameState.bracket.thirdPlace,
                loc: { side: 'bronze', roundIndex: 0, matchIndex: 0 }
            });
        } else if (phaseKey === 'final') {
            result.push({
                match: gameState.bracket.final,
                loc: { side: 'final', roundIndex: 0, matchIndex: 0 }
            });
        }
        return result;
    }

    function getBracketPhaseMatchStats(phaseKey) {
        var entries = getBracketMatchesInPhase(phaseKey);
        var done = 0;
        for (var i = 0; i < entries.length; i++) {
            if (entries[i].match && entries[i].match.status === 'done') done++;
        }
        return { total: entries.length, done: done };
    }

    function isBracketPhaseComplete(phaseKey) {
        var stats = getBracketPhaseMatchStats(phaseKey);
    ...
    function isBracketPhaseOnArena(phaseKey, arenaId) {
        ...
        if (!assigned) {
            return isTournamentHostDevice() && !shouldUseServerMatchSync();
        }
    ...
    function getArenaIdForBracketFight(loc) {
        return getBracketAssignedArena(getBracketPhaseKeyFromLoc(loc)) || readSelectedArenaId();
    }

    function updatePoolSelectOverlayMode() {
        var poolMain = document.getElementById('poolMainContent');
        var bracketMain = document.getElementById('bracketMainContent');
        var qualRow = document.querySelector('#poolSelectOverlay .qualifying-advancers-row');
        var formBtn = document.getElementById('formBracketBtnPool');
        var randomBtn = document.getElementById('randomPoolsTestBtn');
        var schemeBtn = document.getElementById('openBracketSchemeBtn');
        var useBracket = !!gameState.bracket;
        ...
        if (randomBtn) {
            if (useBracket) randomBtn.classList.add('hidden');
            else randomBtn.classList.remove('hidden');
        }
        if (schemeBtn) {
            if (useBracket) {
                schemeBtn.classList.remove('hidden');
                schemeBtn.textContent = isHostAdminOnlyMode() ? 'Показать сетку' : 'Схема сетки';
            } else {
                schemeBtn.classList.add('hidden');
            }
        }
    }
    ...
    function setPoolAssignedArena(poolId, arenaId) {
        ...
        var normalized = normalizeArenaId(arenaId);
        if (normalized) {
            gameState.poolArenaAssignments[poolId] = normalized;
        } else {
    ...
    function hidePoolDetailView() {
        ...
        var bracketDetail = document.getElementById('bracketDetailContent');
        if (main) main.style.display = '';
        if (detail) detail.style.display = 'none';
        if (poolDetail) poolDetail.style.display = '';
        ...
        for (var i = 0; i < matches.length; i++) {
            (function(match) {
                ...
                if (!isNetworkArenaDevice() &&
                    ...
                    if (isHostAdminOnlyMode()) {
                        btn.textContent = match.status === 'done' ? 'Переназначить' : 'Назначить на площадку';
                    } else if (match.status === 'done') {
    ...
    function onPoolArenaAssignChanged() {
        ...
        if (currentDetailPoolId === poolId) {
            renderPoolFightList(poolId);
            if (select) {
                populatePoolArenaSelect(select, getPoolAssignedArena(poolId), true);
            }
        }
        var mainView = document.getElementById('poolSelectMainView');
        if (mainView && mainView.style.display !== 'none') {
            renderHostPoolButtons();
        }

        var normalizedArena = normalizeArenaId(arenaId);
        syncAllNetworkActiveNominationsToServer().then(function() {
            if (!normalizedArena) return;
            return ensureHostRoleForOperations().then(function() {
                return dispatchNextFightForArena(normalizedArena);
            });
        }).catch(function() {
            /* sync уже показал alert */
        });
    }
    ...
    function renderHostBracketPhaseButtons() {
        ...
        for (var i = 0; i < phases.length; i++) {
            (function(phase) {
                ...
                var assigned = getBracketAssignedArena(phase.key);
                if (assigned) label += ' · пл. ' + assigned;
                btn.textContent = label;
        ...
        if (!list.children.length) {
            ...
            empty.textContent = 'В сетке пока нет боёв.';
            list.appendChild(empty);
        }
    }

    function showBracketPhaseDetailView(phaseKey) {
        ...
        if (detail) detail.style.display = 'block';
        if (poolDetail) poolDetail.style.display = 'none';
        if (bracketDetail) bracketDetail.style.display = 'block';

        var title = document.getElementById('bracketDetailTitle');
        if (title) title.textContent = getBracketPhaseLabel(phaseKey);
    ...
    function renderBracketFightList(phaseKey) {
        var list = document.getElementById('bracketFightList');
        if (!list) return;
        list.innerHTML = '';

        var fightArenaId = getBracketAssignedArena(phaseKey) || readSelectedArenaId();
        var entries = getBracketMatchesInPhase(phaseKey);
        if (!entries.length) {
            var empty = document.createElement('li');
            empty.className = 'pool-fight-item pool-fight-empty';
            empty.textContent = 'Бои не сформированы';
            list.appendChild(empty);
            return;
        }
        ...
        for (var i = 0; i < entries.length; i++) {
            (function(entry) {
                ...
                var loc = entry.loc;
                if (!match || !match.fighter1 || !match.fighter2) return;
                if (match.fighter1.isBye || match.fighter2.isBye) return;
                ...
                var showFightBtn = canHostFight && (
                    (match.status === 'done' && canEditMatchOnArena(match, fightArenaId)) ||
                    (match.status === 'in_progress' && normalizeArenaId(match.arenaId) === normalizeArenaId(fightArenaId)) ||
                    (match.status !== 'done' && match.status !== 'in_progress' && canFightBracketMatch(match, loc))
                );
                if (showFightBtn) {
                    var btn = document.createElement('button');
                    btn.type = 'button';
                    ...
                    btn.onclick = function() {
                        ...
                        startNetworkMatch(item, fightArenaId, {
                            reopen: match.status === 'done'
                        });
                    };
                    li.appendChild(btn);
                } else if (!isNetworkMode() && canFightBracketMatch(match, loc) && match.status !== 'done') {
                    var localBtn = document.createElement('button');
                    localBtn.type = 'button';
                    localBtn.className = 'menu-button';
                    localBtn.textContent = 'Бой';
                    localBtn.onclick = function() {
                        hidePoolSelectModal();
                        startBracketMatch(loc);
                    };
                    li.appendChild(localBtn);
                } else if (!isNetworkMode() && match.status === 'done') {
                    var editLocalBtn = document.createElement('button');
                    editLocalBtn.type = 'button';
                    editLocalBtn.className = 'menu-button';
                    editLocalBtn.textContent = 'Изменить';
                    editLocalBtn.onclick = function() {
                        hidePoolSelectModal();
                        startBracketMatch(loc, { reopen: true });
                    };
                    li.appendChild(editLocalBtn);
                }
                list.appendChild(li);
            })(entries[i]);
        }
    ...
    function onBracketArenaAssignChanged() {
        if (!currentDetailBracketPhaseKey) return;
        var select = document.getElementById('bracketArenaAssignSelect');
        var arenaId = select && select.value ? parseInt(select.value, 10) : null;
        var phaseKey = currentDetailBracketPhaseKey;
        setBracketAssignedArena(phaseKey, arenaId);
    ...
    function showHostBracketManagementModal() {
        hidePoolDetailView();
        hideBracketDetailView();
        updatePoolSelectOverlayMode();
        ...
        var title = document.getElementById('poolSelectTitle');
        var arenaRow = document.getElementById('arenaSelectRow');
        if (title) title.textContent = 'Сетка турнира';
    ...
    function showHostPoolManagementModal() {
        showHostPoolManagementModalInner();
    }

    function showHostPoolManagementModalInner() {
        hidePoolDetailView();
        hideBracketDetailView();
        updatePoolSelectOverlayMode();
        ...
        renderHostPoolButtons();
        var backBtn = document.querySelector('#poolSelectOverlay .overlay-toolbar-actions .menu-button.back');
        if (backBtn) {
            backBtn.textContent = isHostAdminOnlyMode() ? 'К номинациям' : 'Назад';
        }
        document.getElementById('poolSelectOverlay').style.display = 'flex';
        updateTournamentBar();
    }

    function arenaHasActiveFightSession() {
        return !!(gameState.activePoolMatchId || gameState.activeBracketMatch);
    }

    function updateArenaScheduleOverlayToolbar() {
        if (!isNetworkArenaDevice()) return;
        var toolbar = document.querySelector('#poolSelectOverlay .overlay-toolbar-actions');
        if (!toolbar) return;
        toolbar.style.display = '';
        var backBtn = toolbar.querySelector('.menu-button.back');
        if (backBtn) {
            backBtn.textContent = arenaHasActiveFightSession() ? 'К бою' : 'Закрыть';
        }
    }

    function returnToArenaFightFromSchedule() {
        hidePoolSelectModal();
        var main = document.getElementById('poolSelectMainView');
        var detail = document.getElementById('poolSelectDetailView');
        if (main) main.style.display = '';
    ...
    function showArenaPoolSelectModal() {
        ...
        updateArenaScheduleOverlayToolbar();
        list.innerHTML = '';

        var localArenaId = normalizeArenaId(getLocalArenaId());
        var hasPools = false;
        var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
        var nomEntries = getNominationsForArena(localArenaId, remote);

        function appendPoolButton(snapshot, nominationId, pool) {
            var assigned = getPoolAssignedArenaInSnapshot(snapshot, pool.id);
            if (!assigned || assigned !== localArenaId) return;
            hasPools = true;
            (function(pid, pnum, nomId, nomName, snap) {
                var done = 0;
                var total = 0;
                var matches = (snap.poolMatches && snap.poolMatches[pid]) || [];
                total = matches.length;
                for (var mi = 0; mi < matches.length; mi++) {
                    if (matches[mi].status === 'done') done++;
                }
                var btn = document.createElement('button');
                btn.type = 'button';
                btn.className = 'menu-button';
                var label = (nomName ? nomName + ' · ' : '') + 'Пул ' + pnum;
                if (total) label += ' (' + done + '/' + total + ')';
                if (total && done === total) label += ' ✓';
                btn.textContent = label;
                btn.onclick = function() {
                    currentArenaViewNominationId = nomId;
                    applySnapshotToGameState(snap, {
                        nominationId: nomId,
                        nominationName: nomName
                    });
                    showPoolDetailView(pid);
                };
                list.appendChild(btn);
            })(pool.id, pool.number, nominationId, snap.nominationName || '', snapshot);
        }

        for (var ni = 0; ni < nomEntries.length; ni++) {
            var snap = nomEntries[ni].snapshot;
            if (!isPoolStageSnapshot(snap)) continue;
            var pools = snap.pools || [];
            for (var pi = 0; pi < pools.length; pi++) {
                appendPoolButton(snap, nomEntries[ni].nominationId, pools[pi]);
            }
        }
        if (!nomEntries.length && isPoolStageActive()) {
            for (var i = 0; i < gameState.pools.length; i++) {
                appendPoolButton(gameState, gameState.activeNominationId, gameState.pools[i]);
            }
        }
    ...
    function showArenaBracketScheduleModal() {
        ...
        function appendBracketPhasesFromSnapshot(snapshot, nominationId) {
            if (!snapshot || !snapshot.bracket) return;
            var savedBracket = gameState.bracket;
            var savedAssign = gameState.bracketArenaAssignments;
            gameState.bracket = snapshot.bracket;
            gameState.bracketArenaAssignments = snapshot.bracketArenaAssignments || {};
            var phases = enumerateBracketPhases();
            var nomName = snapshot.nominationName || '';
            for (var i = 0; i < phases.length; i++) {
                (function(phase, snap, nomId, nName) {
                    ...
                    btn.type = 'button';
                    btn.className = 'menu-button';
                    var label = (nName ? nName + ' · ' : '') + phase.label;
                    if (stats.total) label += ' (' + stats.done + '/' + stats.total + ')';
                    if (isBracketPhaseComplete(phase.key)) label += ' ✓';
                    btn.textContent = label;
                    btn.onclick = function() {
                        ...
                        applySnapshotToGameState(snap, {
                            nominationId: nomId,
                            nominationName: nName
                        });
                        showBracketPhaseDetailView(phase.key);
                    };
                    list.appendChild(btn);
                })(phases[i], snapshot, nominationId, nomName);
            }
            gameState.bracket = savedBracket;
            gameState.bracketArenaAssignments = savedAssign;
        }

        for (var ni = 0; ni < nomEntries.length; ni++) {
            if (!isBracketStageSnapshot(nomEntries[ni].snapshot)) continue;
            appendBracketPhasesFromSnapshot(
                nomEntries[ni].snapshot,
                nomEntries[ni].nominationId
            );
        }
        if (!nomEntries.length && isBracketStageActive()) {
            appendBracketPhasesFromSnapshot(gameState, gameState.activeNominationId);
        }
    ...
    function openArenaScheduleView() {
        ...
        var arenaId = getLocalArenaId();
        var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
        if (arenaHasPendingBracketWork(arenaId, remote)) {
            showArenaBracketScheduleModal();
        } else if (arenaHasPendingPoolWork(arenaId, remote)) {
            showArenaPoolSelectModal();
        } else {
    ...
    function showArenaMatchSelectModal() {
        if (isNetworkArenaDevice()) {
            if (isArenaStageCompleteForTerminal() && !arenaHasActiveFightSession()) {
                showArenaStageCompleteOverlay();
                return;
            }
            var remoteArena = typeof NetworkSync !== 'undefined'
                ? NetworkSync.getState().lastRemoteState
                : null;
            var localArena = getLocalArenaId();
            if (arenaHasPendingBracketWork(localArena, remoteArena)) {
                showArenaBracketScheduleModal();
                return;
            }
            if (arenaHasPendingPoolWork(localArena, remoteArena)) {
                showArenaPoolSelectModal();
                return;
            }
            var remote = typeof NetworkSync !== 'undefined' ? NetworkSync.getState().lastRemoteState : null;
    ...
    function startNetworkMatch(item, arenaId, options) {
        ...
        if (!shouldUseServerMatchSync()) {
            beginFight();
            return;
        }

        ensureHostRoleForOperations().then(function() {
            return ensureNominationSyncedBeforeClaim(nominationId);
        }).then(function() {
            if (item.type === 'pool') {
                return NetworkSync.claimMatch('pool', {
                    poolId: item.poolId,
            ...
            alert(formatDispatchError(err) || 'Не удалось занять бой');
            if (isHostAdminOnlyMode()) {
                refreshPoolSelectOverlayUi();
            } else if (!isNetworkArenaDevice()) {
    ...
    function startPlayoffFromPools() {
        ...
        if (gameState.activeTournamentId && gameState.activeNominationId &&
            typeof MeisterTournaments !== 'undefined' &&
            MeisterTournaments.setNominationNetworkActive) {
            MeisterTournaments.setNominationNetworkActive(
                gameState.activeTournamentId,
                gameState.activeNominationId,
                true
            );
        }
    ...
    function updateTournamentBar() {
        ...
        var text = '';
        if (gameState.activeNominationName) {
            text += gameState.activeNominationName;
        } else {
            text += 'Плей-офф';
        }
        if (gameState.activePoolId) {
    ...
    function maybeRunBracketPreviewFromUrl() {
        if (maybeRunBracketDisplayViewFromUrl()) return;
        var params = new URLSearchParams(window.location.search);
    ...
    function createBracketMatchEl(match, loc, matchCount) {
        ...
        } else if (!isHostAdminOnlyMode() && match.status === 'done') {
        ...
        } else if (!isHostAdminOnlyMode() && canFightBracketMatch(match, loc)) {
            var btn = document.createElement('button');
            btn.type = 'button';
    ...
    function saveActiveBracketMatchResult() {
        ...
        var completedArenaId = normalizeArenaId(match.arenaId) ||
            getBracketAssignedArena(getBracketPhaseKeyFromLoc(loc)) ||
            gameState.activeArenaId ||
            readSelectedArenaId();
    ...
    function backFromPoolSelect() {
        if (isNetworkArenaDevice()) {
            if (arenaHasActiveFightSession()) {
                returnToArenaFightFromSchedule();
                return;
            }
        ...
        hidePoolSelectModal();
        if (isHostAdminOnlyMode() && gameState.playoffStarted) {
            hideSecretaryTerminal();
            if (typeof global.backFromNominationSetup === 'function') {
                global.backFromNominationSetup();
            }
            return;
        }
        document.getElementById('poolsOverlay').style.display = 'flex';
    ...
    function playoffGoBack() {
        ...
        if (poolSelectVisible) {
            if (isNetworkArenaDevice()) {
                if (currentDetailPoolId) {
                    hidePoolDetailView();
                } else if (currentDetailBracketPhaseKey) {
                    hideBracketDetailView();
                } else if (arenaHasActiveFightSession()) {
                    returnToArenaFightFromSchedule();
                } else {
                    closeArenaPoolSelectModal();
                }
    ...
    global.TournamentPlayoff = {
        ...
        openBracketDisplayWindow: openBracketDisplayWindow,
        getBracketDisplayUrl: getBracketDisplayUrl,
        initBracketDisplayView: initBracketDisplayView,
        applyBracketDisplayFromRemote: applyBracketDisplayFromRemote,
        ...
        isHostAdminOnlyMode: isHostAdminOnlyMode,
        ensureHostAdminPanelVisible: ensureHostAdminPanelVisible,
        hideSecretaryTerminal: hideSecretaryTerminal,
        showArenaMatchSelectModal: showArenaMatchSelectModal,
        syncTournamentToServer: syncTournamentToServer,
        syncAllNetworkActiveNominationsToServer: syncAllNetworkActiveNominationsToServer,
        dispatchForAllActiveArenas: dispatchForAllActiveArenas,
    ...
    global.onBracketArenaAssignChanged = onBracketArenaAssignChanged;
    global.showHostBracketManagementModal = showHostBracketManagementModal;
    ...
    function openBracketView() {
        if (isHostAdminOnlyMode() && gameState.bracket) {
            showHostBracketManagementModal();
            return;
        }
        openBracketSchemeView();
    ...
    global.openBracketSchemeView = openBracketSchemeView;
    global.openBracketDisplayWindow = openBracketDisplayWindow;
    global.onAdminArenaChanged = onAdminArenaChanged;
    global.TournamentPlayoff.openBracketView = openBracketView;
    global.TournamentPlayoff.openBracketSchemeView = openBracketSchemeView;
